...
//!
//! Vendored locally (cord-commons fork remedy, same class of fix as the
//! `sp_core::{blake2_256,twox_64}` -> `sp-crypto-hashing` relocation already applied to
//! `pallets/{airdrop,pgas,coinage}`): this pallet's `indiv-support` dependency resolves to
//! `github.com/dhiway/cord`'s `indiv-support` crate on the `cord-commons-2606` branch (the whole
//! workspace's deliberate, already-established substitution so `Members`/`AppendOnlyMembers`/etc.
//! unify across every individuality-community pallet consumed here — see cord-commons's own root
//! `Cargo.toml` dependency comments), which does not carry this crate's own native `support/src/
//! labels.rs` module. Copied byte-for-byte from `paritytech/individuality-community`'s
//! `support/src/labels.rs` (Apache-2.0, upstream license header preserved) — zero functional
//! change, pure `no_std`-safe string validation with no dependency on anything `indiv-support`
//! itself provides.
